...
GAMMA                   = 0.88
LR_AC                   = 1/6.e3 # learning rate for actor
GLOBAL_REWARD           = []
GLOBAL_EP               = 0
START_EP                = GLOBAL_EP
...
nIter = UPDATE_GLOBAL_ITER*TIME_FACTOR + WALKING*250
...
shoulderVec = np.matrix([[0.1350, -0.1350, 0.1350, -0.1350, 0.1350, -0.1350], [0.0970, 0.0970, 0.0000, -0.0000, -0.0970, -0.0970], [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000]])
...
cpg0 = {
    ...
    'done': np.array([False for _ in range(6)]),
}
...
class ACNet(object):
    def __init__(self, scope):
        with tf.variable_scope(str(scope)+'/qvalues'):
            ...
            if RNN_USE:
                self.policy, self.value, self.state_out, self.state_in, self.state_init, self.too_high, self.on_ground = self._build_net(self.inputs)
            else:
                self.policy, self.value, self.too_high, self.on_ground = self._build_net(self.inputs)
        if(scope!=GLOBAL_NET_SCOPE):
            self.actions = tf.placeholder(shape=[None], dtype=tf.int32)
            self.actions_onehot = tf.one_hot(self.actions, a_size, dtype=tf.float32)
            self.target_v = tf.placeholder(tf.float32, [None], 'Vtarget')
            self.target_too_high = tf.placeholder(tf.float32, [None], 'target_too_high')
            self.target_on_ground = tf.placeholder(tf.float32, [None], 'target_on_ground')
            ...
            self.too_high_loss = - tf.reduce_sum(self.target_too_high*tf.log(tf.clip_by_value(tf.reshape(self.too_high, shape=[-1]),1e-10,1.0))+(1-self.target_too_high)*tf.log(tf.clip_by_value(1-tf.reshape(self.too_high, shape=[-1]),1e-10,1.0)))
            self.on_ground_loss = - self.decay_on_ground * tf.reduce_sum(self.target_on_ground*tf.log(tf.clip_by_value(tf.reshape(self.on_ground, shape=[-1]),1e-10,1.0))+(1-self.target_on_ground)*tf.log(tf.clip_by_value(1-tf.reshape(self.on_ground, shape=[-1]),1e-10,1.0)))
            ...
            self.loss = 0.5 * self.value_loss + 0.5 * self.too_high_loss + 0.5 * self.on_ground_loss + self.policy_loss - self.entropy * 0.01
    ...
    def _build_net(self,inputs):
        ...
        h0 = layers.fully_connected(inputs=inputs, num_outputs=128, activation_fn=None)
        conv1 = tf.layers.conv1d(inputs=tf.reshape(h0, shape=(-1,128,1)), filters=1, kernel_size=1, strides=1, use_bias=False)
        ...
        h4 = layers.fully_connected(inputs=h3,     num_outputs=128, activation_fn=None)
        ...
        policy = layers.fully_connected(inputs=rnn_out, num_outputs=a_size, biases_initializer=None, activation_fn=tf.nn.softmax, weights_initializer=normalized_columns_initializer(1.0))
        value = layers.fully_connected(inputs=rnn_out, num_outputs=1, biases_initializer=None, activation_fn=None, weights_initializer=normalized_columns_initializer(1.0))
        too_high = layers.fully_connected(inputs=rnn_out, num_outputs=1, biases_initializer=None, activation_fn=tf.sigmoid, weights_initializer=normalized_columns_initializer(1.0))
        on_ground = layers.fully_connected(inputs=rnn_out, num_outputs=1, biases_initializer=None, activation_fn=tf.sigmoid, weights_initializer=normalized_columns_initializer(1.0))
        ...
        while True:
            ...
            if episodeStep == 0:
                ...
                if int(pt_h > 0.5) != t_h and t_h != 0.5:
                    ...
                    too_high_wrongs       += 1
                    train_value            = 0
                elif int(po_g > 0.5) != o_g and o_g != 0.5:
                    a                      = np.random.choice(range(a_size))
                    on_ground_wrongs      += 1
                    train_value            = 0
                ...
                if not cpg['done'].all():
                    ...
                    episodetmp=[s1,a,v,t_h,o_g,train_value,Zerr]
                ...
                if episodeStep == UPDATE_GLOBAL_ITER or cpg['done'].all():
                    print('({})Worker {} updating global net'.format(episodeStep, self.workerID))
                    ...
                    if RNN_USE:
                        ...
                        s1_values[i_buf] = SESS.run(self.local_AC.value, {self.local_AC.inputs: np.reshape(np.array(s1),[1,s_size])})[0,0]
                        
                    i_rand = np.random.randint(min(int((GLOBAL_EP-START_EP)/N_WORKERS)+1,NUM_BUFFERS))
                    episodeReward,v_l,p_l,e_l,g_n,v_n,th_l,og_l = self.train(episode_buffers[i_rand],s1_values[i_rand],Zerr)
                    SESS.run(self.pull_global)
                    ...
                    episodeReward, GLOBAL_EP = 0, GLOBAL_EP+1
                    episodeStep = 0
                    EPISODE_FINISHED = True

                    # Tensorboard stuffs
                    self.tensorboardPlot(criticLoss, actorLoss, th_l, too_high_wrongs, og_l, on_ground_wrongs)
                    if GLOBAL_EP % 6 == 0:
            ...
            # Synchronize threads before writing into cpg['done']
            self.groupLock.acquire(self.groupNumber,self.name)
            if episodeStep >= cpg['zHistory'].shape[1]:
                cpg['done'][self.workerID] = (abs(Zerr) < ZERROR_THRESHOLD)
                assert(cpg['done'][self.workerID] == (abs(Zerr) < ZERROR_THRESHOLD))
            self.groupLock.release(self.groupNumber,self.name)

    def closestPerfectAction(self):
        a = cpg['dTheta2GS'][self.workerID]
        return np.argmin( (np.array(actionList) - a)**2 )

    def performAction(self, action):
#        print('Agent {} updating dTheta2: {}'.format(self.workerID, actionList[action]))
        cpg['dTheta2'][self.workerID] = actionList[action]

    def medfilter(self, array, win_size):
        filtered_array = array.copy()

        for i in range(len(array)):
            filtered_array[i] = np.mean(array[ max(0,i-win_size):min(len(array)-1,i+win_size+1) ])
    ...
    def train(self, rollout, bootstrap_value, Zerr1):
        ...
        values          = rollout[:,2]
        too_highs       = rollout[:,3]
        on_grounds      = rollout[:,4]
        train_values    = rollout[:,5]
        #rewards         = rollout[:,-1]
        Zerrors         = np.array(list(rollout[:,-1]) + [Zerr1])

        # Reward filtering
        filtered_err        = self.medfilter(Zerrors, FILTER_WIN)
        rewards             = ACTION_COST + np.clip( (filtered_err[:-1] - filtered_err[1:]) / dt, -5.0, 5.0 )
        rewards            *= np.asarray(on_grounds, dtype=float) # cancel rewards for legs in the air (no responsibility)
        ...
        if RNN_USE:
            ...
            self.local_AC.inputs:np.stack(observations[sampleInd]),
    ...
    def getReward(self, oldZErr, dt):
        # Leg-specific vertical error
        heightFac, verticalError = 10.0, cpg['shoulderZError'][self.workerID] - cpg['zErr']
        zError    = (heightFac * verticalError)**2

        on_ground = int(cpg['jacobianTorques'][2,self.workerID] > TORQUE_THRES) #or cpg['on_ground'][self.workerID]
        dZErr = np.clip((oldZErr-zError)/dt, -5.0, 5.0) # minus sign needed
        reward = on_ground * dZErr
    ...
    def tensorboardPlot(self, criticLoss, actorLoss, too_high_loss, too_high_wrongs, on_ground_loss, on_ground_wrongs):
        ...
        if GLOBAL_EP >= 12:
            ...
            meanCriticLoss = np.max(np.mean(criticLoss[-12:]))
            meanActorLoss = np.max(np.mean(actorLoss[-12:]))
            too_high_accuracy = (UPDATE_GLOBAL_ITER-too_high_wrongs)/float(UPDATE_GLOBAL_ITER)
            on_ground_accuracy = (UPDATE_GLOBAL_ITER-on_ground_loss)/float(UPDATE_GLOBAL_ITER)
            summary=tf.Summary()
            ...
            summary.value.add(tag='Losses/Too_High Body Loss', simple_value=float(too_high_loss))
            summary.value.add(tag='Losses/Too_High Body Accuracy', simple_value=float(too_high_accuracy))
    ...
    def getStateAction(self, init_state):
        ## State definition
        state           = [
                            2/np.pi*cpg['legs'][0,shoulders1][self.workerID],
                            2/np.pi*cpg['legs'][0,shoulders2][self.workerID],
                            2/np.pi*cpg['legs'][0,elbows][self.workerID],
                            10*(cpg['shoulderZError'][self.workerID] - cpg['zErr']),
                            0.2*cpg['torques'][0,self.workerID],
                            0.2*cpg['torques'][1,self.workerID],
                            0.2*cpg['torques'][2,self.workerID]
                          ]
        # Action selection
        if RNN_USE:
            ...
            a_dist, value, pred_too_high, pred_on_ground = SESS.run([self.local_AC.policy,self.local_AC.value, self.local_AC.too_high, self.local_AC.on_ground], feed_dict={self.local_AC.inputs:np.reshape(np.array(state),[1,s_size])})
            rnn_state = 0
        a = np.random.choice(a_dist[0],p=a_dist[0])
        a = np.argmax(a_dist==a)
        
        verticalError = cpg['shoulderZError'][self.workerID] - cpg['zErr']
        too_high = int(verticalError > 0) # cpg['zErr'] = cpg['bodyHeight'] - np.median(cpg['zHistory'])
        if abs(verticalError) < TH_DEADZONE:
            too_high = 0.5

        on_ground = int(cpg['jacobianTorques'][2,self.workerID] > 0.) #or cpg['on_ground'][self.workerID]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not WALKING:
            readyForNextEpisode = 'n'
            while not (readyForNextEpisode=='y'):
                cpg = configCreator.cpgConfig(cpg, randHeight=0.)
                snakeMonster.setAngles(cpg['legs'][0])
                readyForNextEpisode = 'y' # input('Ready for next Episode (y/n)?  ') # uncomment this to add verification step
        ...
        for t in range(nIter):
            ...
            if t >= cpg['initLength']:
                ...
                for leg in range(6):
                    cpg['jacobianTorques'][:,leg] = np.matmul(cpg['pose'].T, np.linalg.lstsq(J[leg,:3,:].T, cpg['torques'][:,leg])[0])
...
#                cpg['dTheta2GS'] = jacobianAngleCorrection(cpg)[1]
                if (t % TIME_FACTOR) == 0:
                    ...
                    groupLock.acquire(0,'main')#lock here
                    groupLock.release(0,'main')# cpg['done'] writing
