...
NsavedModels = 50  # keep ALL models (otherwise, change to 5 or 10)
s_size = 7*6
actionList = [-1., 0., 1.]
...
class ACNet(object):
    ...
    def _build_net(self, inputs):
        ...
        h0 = layers.fully_connected(
            inputs=inputs, num_outputs=256, activation_fn=None)
        conv1 = tf.layers.conv1d(inputs=tf.reshape(
            h0, shape=(-1, 256, 1)), filters=1, kernel_size=1, strides=1, use_bias=False)

        h1 = layers.fully_connected(inputs=inputs, num_outputs=256)
        h2 = layers.fully_connected(inputs=h1,     num_outputs=256)
        h3 = layers.fully_connected(inputs=h2,     num_outputs=256)
        h4 = layers.fully_connected(inputs=h3,     num_outputs=256, activation_fn=None)

        res1 = tf.nn.relu(layers.flatten(conv1) + h4)
        h5 = layers.fully_connected(inputs=res1,  num_outputs=256)
...
class Worker(object):
        ...
        while True:
            ...
            if episodeStep == 0:
                ...
                if int(pt_h > 0.5) != t_h and t_h != 0.5:
                    ...
                    if GLOBAL_EP < STOP_ON_GROUND:
                        a = np.random.choice(range(a_size))
                        train_value = 0
                    on_ground_wrongs += np.sum(np.asarray( (np.logical_and(np.asarray(po_g > 0.5, dtype=int) != o_g, o_g != 0.5)), dtype=int)) / 6.
    ...
    def train(self, rollout, bootstrap_value, Zerr1):
        ...
        if GLOBAL_EP > STOP_ON_GROUND:
            decay_on_ground = 0.0
        elif GLOBAL_EP > TRAIN_ON_GROUND:
    ...
    def getZerror(self):
        ...
        heightFac, verticalErrors = 10.0, [(err - cpg['zErr']) for err in cpg['shoulderZError']] 
        return sum([ (heightFac * ve)**2 for ve in verticalErrors ]) / 6.
