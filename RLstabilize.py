...
GAMMA                   = 0.88
LR_AC                   = 1/6.e4 # learning rate for actor
GLOBAL_REWARD           = []
GLOBAL_EP               = 0
START_EP                = GLOBAL_EP
N_WORKERS               = 6 #should = multiprocessing.cpu_count()
model_path              = './model_online'
#model_path              = './model_keep'
NsavedModels            = 5 # keep ALL models (otherwise, change to 5 or 10)
s_size                  = 7
actionList              = [-0.3, 0., 0.3]
...
#                    train_value            = 0
                ...
                if episodeStep == UPDATE_GLOBAL_ITER:
                    print('Worker {} updating global net'.format(self.workerID))
...
#                        saver.save(SESS, model_path+'/model-'+str(GLOBAL_EP)+'.cptk')
    ...
    def closestPerfectAction(self):
        a = self.cpg['dTheta2GS'][self.workerID]
...
#        print('Agent {} updating dTheta2: {}'.format(self.workerID, actionList[action]))
    ...
    def train(self, rollout, bootstrap_value, Zerr1):
        ...
        rewards             = np.clip( (filtered_err[:-1] - filtered_err[1:]) / dt, -5.0, 5.0 )
        ...
        sampleInd = np.sort(np.random.choice(advantages.shape[0], size=(EPISODE_SAMPLES,), replace=False))
    ...
    def getReward(self, oldZErr, dt):
        ...
        heightFac, verticalError = 10.0, self.cpg['shoulderZError'][self.workerID] - self.cpg['zErr']
    ...
    def getStateAction(self, init_state):
        ## State definition
        state           = [
                            2/np.pi*self.cpg['legs'][0,shoulders1][self.workerID],
                            2/np.pi*self.cpg['legs'][0,shoulders2][self.workerID],
                            2/np.pi*self.cpg['legs'][0,elbows][self.workerID],
                            10*(self.cpg['shoulderZError'][self.workerID] - self.cpg['zErr']),
                            0.2*self.cpg['torques'][0,self.workerID],
                            0.2*self.cpg['torques'][1,self.workerID],
                            0.2*self.cpg['torques'][2,self.workerID]
                          ]
        # Action selection
        if RNN_USE:
        ...
        verticalError = self.cpg['shoulderZError'][self.workerID] - self.cpg['zErr']
